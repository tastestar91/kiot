...
MQTT_SUB_TOPICS = [
	("himpel/ack/#", 0), ("kerv/ack/#", 0), ("kaic1/ack/#", 0),
	("damper/ack/#", 0), ("samsungcnt/ack/#", 0), ("hum/ack/#", 0),
	("kesr/ack/#", 0), ("hvac/ack/#", 0) 
]	# [(topic, qos), ]

SENSOR_FIELDS = [
	"power", "air_volume", "exh_mode", "auto_mode", "filter_alarm", "air_mode"
]

SENSOR_FIELDS_AHU = [
	"power", "air_volume", "fire_alarm", "auto_mode", "filter_alarm"
]

SENSOR_FIELDS_KWG = [
	"power", "air_volume", "water_alarm", "auto_mode", "filter_alarm", "dev_stat"
]

SENSOR_FIELDS_HUM = [
	"power", "air_volume", "auto_mode", "exh_mode", "filter_alarm",
	"hum_rtemp", "hum_rhumi", "hum_otemp", "hum_ohumi", "hum_co2", "hum_pm10", "hum_pm25"
]
...
def process_message(msg):
	try:
		...
		if topic_tree[0] == "himpel":		# Model: JNT
			data_str = message[2:-2]
			data_sensor = SENSOR_FIELDS

		elif (topic_tree[0] == "kerv") or (topic_tree[0] == "kaic1") or (topic_tree[0] == "kesr") or (topic_tree[0] == "hvac"):		# Model: TAES, KWV-AIC1, KESR
			data_str = message[4:-2]
			data_sensor = SENSOR_FIELDS

		elif topic_tree[0] == "damper":		# Model: AHU
			data_str = message[4:-2]
			data_sensor = SENSOR_FIELDS_AHU

		elif topic_tree[0] == "samsungcnt":	# Model: KWG-ST1
			data_str = message[4:-2]
			data_sensor = SENSOR_FIELDS_KWG

		elif topic_tree[0] == "hum":		# Model: HUM
			data_str = message[4:-2]
			data_sensor = SENSOR_FIELDS_HUM
		...
		if topic_tree[0] == "hum":		# Model: HUM
			...
			for i in range(0, len(data_sensor)):
				...
				payload[data_sensor[i]] = data_str[idx:idx+flen]
				idx = idx + flen
		else:
			for i in range(0, len(data_sensor)):
				if data_str[i*2:i*2+2] != "NA":
					payload[data_sensor[i]] = data_str[i*2:i*2+2]
