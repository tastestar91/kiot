...
const debug = 
	require('debug')('ingress-router:postprocessor:psp-kw-ctl-v1-response');
...
function postprocess(irData) {
	return new Promise(async (resolve, reject) => {
		try {
			...
			irData.response = {
				statusCode: 200,
				message: `<kweather_iot><result>${result}</result><reset>${reset}</reset><update>${update}</update><datetime>${datetime}</datetime></kweather_iot>`
			};
